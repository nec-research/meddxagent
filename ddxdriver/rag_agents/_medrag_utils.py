...
class CustomizeSentenceTransformer(
    ...
    def _load_auto_model(self, model_name_or_path):
        """
        Creates a simple Transformer + CLS Pooling model and returns the agents
        """
        # print(
    ...
    with torch.no_grad():
        for fname in tqdm.tqdm(fnames):
            ...
            if "specter" in model_name.lower():
                texts = [
                    model.tokenizer.sep_token.join([item["title"], item["contents"]])
                    for item in texts
                ]
            elif "contriever" in model_name.lower():
                texts = [
                    ". ".join([item["title"], item["contents"]])
                    .replace("..", ".")
                    .replace("?.", "?")
                    for item in texts
                ]
            ...
            # print(
            #     f"Encoding a subset of {subset} docs out of the {len(texts)} docs in chunk: {fname}"
            # )
            import time
...
class Retriever:
    ...
    def __init__(
        ...
        db_dir="ddxdriver/rag_agents/corpus",
        **kwarg,
    ):
        self.retriever_name = retriever_name
        if corpus_name not in corpus_names:
            raise ValueError(
                "Invalid corpus name. Should be among PubMed, Textbooks, StatPearls, Wikipedia, MedCorp, PubMed_all, All"
            )
    ...
    def get_relevant_documents(self, question, k=32, **kwarg):
        ...
        if "bm25" in self.retriever_name.lower():
            ...
            indices = [
                {"source": "_".join(h.docid.split("_")[:-1]), "index": eval(h.docid.split("_")[-1])}
                for h in hits
            ]
        else:
            with torch.no_grad():
...
class RetrievalSystem:
    ...
    def merge(self, texts, scores, k=32, rrf_k=100):
        ...
        if len(texts) == 1:
            ...
            texts = [
                dict((key, item[1][key]) for key in ("id", "title", "contentss"))
